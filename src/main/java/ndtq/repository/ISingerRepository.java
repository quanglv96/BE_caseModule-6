...
public interface ISingerRepository extends JpaRepository<Singer, Long> {
    ...
    @Modifying
    @Query(value = "INSERT INTO song_singer (id_song, id_singer) VALUES (?1, ?2);",nativeQuery = true)
    void addSingerSong(Long idSong, Long idSinger);
    @Query(value = "SELECT count(*) FROM song_singer where id_song=?1 and id_singer=?2" ,nativeQuery = true)
